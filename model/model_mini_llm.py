...
class Attention(nn.Module):
    ...
    def __init__(self, args:mini_llm_config):
        super().__init__()
...
class FeedForward(nn.Module):
    ...
    结构: Gate(x) * Up(x) -> Down
    """
    def __init__(self, config: mini_llm_config):
        super().__init__()
...
class MINI_LLMBlock(nn.Module):
    """
    Transformer 块：Self-Attention + FeedForward
    采用 Pre-Norm 结构（Norm before attention/mlp）
    """

    def __init__(self, layer_id: int, config: mini_llm_config):
        super().__init__()
...
class MINI_LLMModel(nn.Module):
    ...
    def __init__(self, config:mini_llm_config):
        ...
        # Transformer Blocks
        self.layers = nn.ModuleList([MINI_LLMBlock(l, config) for l in range(self.num_hidden_layers)])
...
class MINI_LLMForCausalLM(PreTrainedModel, GenerationMixin):
    """
    MINI_LLM 因果语言模型（用于文本生成）
    在 MINI_LLMModel 基础上添加 Language Modeling Head
    """
    config_class = mini_llm_config

    def __init__(self, config: mini_llm_config = None):
        self.config = config or mini_llm_config()
        super().__init__(self.config)
        
        # Transformer 主体
        self.model = MINI_LLMModel(self.config)
