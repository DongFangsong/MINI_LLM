...
from model.config import mini_llm_config
from model.model_mini_llm import MINI_LLMForCausalLM
...
def train_epoch(epoch, loader, iters, start_step=0, swanlab=None, 
                total_steps=None, warmup_steps=None, full_save_dir=None, args=None):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MINI_LLM Pretraining (Single GPU)")
    ...
    parser.add_argument("--data_path", type=str, default={"C:\\Users\\Fang Sir\\Desktop\\Project_LLM\\MINI_LLM\\train"}, help="预处理后的.bin文件路径")
    parser.add_argument('--from_weight', default='none', type=str, help="基于哪个权重训练，为none则从头开始")
    parser.add_argument('--from_resume', default=0, type=int, choices=[0, 1], help="是否自动检测&续训（0=否，1=是）")
    parser.add_argument("--use_swanlab", type=int, default=1, choices=[0, 1], help="是否使用swanlab（0=否，1=是）")
    parser.add_argument("--swanlab_project", type=str, default="MINI_LLM-Pretrain", help="swanlab项目名")
    ...
    # ========== 1. 配置目录、模型参数、检查 ckp ==========
    lm_config = mini_llm_config(hidden_size=args.hidden_size, num_hidden_layers=args.num_hidden_layers)
    ...
    if args.use_swanlab:
        import swanlab
        swanlab.login(api_key="VZmueCVyxWOl9cQE8IZKs")
    ...
    if args.eval_bench == 1 and tokenizer is not None and start_epoch == 0 and start_step == 0:
        ...
        model.eval()
        c3_path = 'C:\\Users\\Fang Sir\\Desktop\\Project_LLM\\MINI_LLM\\dataset\\clue_c3_eval_500.jsonl'
        xcopa_path = 'C:\\Users\\Fang Sir\\Desktop\\Project_LLM\\MINI_LLM\\dataset\\xcopa_zh_merged.jsonl'
        eval_results = run_benchmark(model, tokenizer, c3_path, xcopa_path)
        if swanlab_run:
